define("gallery/tablesorter/2.13.3/widgets-debug", [ "$-debug" ], function(require, exports, module) {
...
*/
        ts.storage = function(table, key, val, options) {
            var d, k, ls = false, v = {}, c = table.config, id = options && options.id || $(table).attr(options && options.group || "data-table-group") || table.id || $(".tablesorter").index($(table)), url = options && options.url || $(table).attr(options && options.page || "data-table-page") || c && c.fixedUrl || window.location.pathname;
        ...
        ts.addHeaderResizeEvent = function(table, disable, options) {
            var defaults = {
                timer: 250
            }, o = $.extend({}, defaults, options), c = table.config, wo = c.widgetOptions, headers, checkSizes = function() {
                wo.resize_flag = true;
                headers = [];
        ...
        ts.addWidget({
            ...
            format: function(table, c, wo) {
                var time, klass, $el, $tar, t = ts.themes, $t = c.$table, theme = c.theme !== "default" ? c.theme : wo.uitheme || "jui", o = t[t[theme] ? theme : t[wo.uitheme] ? wo.uitheme : "jui"], $h = c.$headers, sh = "tr." + (wo.stickyHeaders || "tablesorter-stickyHeader"), rmv = o.sortNone + " " + o.sortDesc + " " + o.sortAsc;
                if (c.debug) {
                    time = new Date();
                }
                ...
                if (!$t.hasClass("tablesorter-" + theme) || c.theme === theme || !table.hasInitialized) {
                    ...
                    if (t.length) {
                        t.find("tr").addClass(o.footerRow).children("th, td").addClass(o.footerCells);
                    }
                    // update header classes
                    $h.addClass(o.header).filter(":not(.sorter-false)").bind("mouseenter.tsuitheme mouseleave.tsuitheme", function(e) {
            ...
            remove: function(table, c, wo) {
                var $t = c.$table, theme = typeof wo.uitheme === "object" ? "jui" : wo.uitheme || "jui", o = typeof wo.uitheme === "object" ? wo.uitheme : ts.themes[ts.themes.hasOwnProperty(theme) ? theme : "jui"], $h = $t.children("thead").children(), rmv = o.sortNone + " " + o.sortDesc + " " + o.sortAsc;
                $t.removeClass("tablesorter-" + theme + " " + o.table).find(ts.css.header).removeClass(o.header);
                $h.unbind("mouseenter.tsuitheme mouseleave.tsuitheme").removeClass(o.hover + " " + rmv + " " + o.active).find(".tablesorter-filter-row").removeClass(o.filterRow);
        ...
        ts.addWidget({
            ...
            format: function(table, c, wo) {
                var $tb, $tr, $td, $t, time, last, rmv, i, k, l, $tbl = c.$table, b = c.$tbodies, list = c.sortList, len = list.length, // keep backwards compatibility, for now
            ...
            remove: function(table, c, wo) {
                var k, $tb, b = c.$tbodies, rmv = (wo.columns || [ "primary", "secondary", "tertiary" ]).join(" ");
        ...
        ts.addWidget({
            ...
            remove: function(table, c, wo) {
                var tbodyIndex, $tbody, $table = c.$table, $tbodies = c.$tbodies;
                $table.removeClass("hasFilters").unbind("addRows updateCell update updateComplete appendCache search filterStart filterEnd ".split(" ").join(".tsfilter ")).find(".tablesorter-filter-row").remove();
        ...
        ts.filter = {
            ...
            types: {
                ...
                operators: function(filter, iFilter, exact, iExact, cached, index, table, wo, parsed) {
                    if (/^[<>]=?/.test(iFilter)) {
                        var cachedValue, result, c = table.config, query = ts.formatFloat(iFilter.replace(ts.filter.regex.operators, ""), table), parser = c.parsers[index], savedSearch = query;
                ...
                and: function(filter, iFilter, exact, iExact) {
                    if (/\s+(AND|&&)\s+/g.test(filter)) {
                        var query = iFilter.split(/(?:\s+(?:and|&&)\s+)/g), result = iExact.search($.trim(query[0])) >= 0, indx = query.length - 1;
                        while (result && indx) {
                ...
                range: function(filter, iFilter, exact, iExact, cached, index, table, wo, parsed) {
                    if (/\s+(-|to)\s+/.test(iFilter)) {
                        var result, c = table.config, query = iFilter.split(/(?: - | to )/), // make sure the dash is for a range and not indicating a negative number
                        range1 = ts.formatFloat(query[0].replace(ts.filter.regex.nondigit, ""), table), range2 = ts.formatFloat(query[1].replace(ts.filter.regex.nondigit, ""), table);
                        // parse filter value in case we're comparing numbers (dates)
                ...
                wild: function(filter, iFilter, exact, iExact, cached, index, table) {
                    if (/[\?|\*]/.test(iFilter) || /\s+OR\s+/.test(filter)) {
                        var c = table.config, query = iFilter.replace(/\s+OR\s+/gi, "|");
                ...
                fuzzy: function(filter, iFilter, exact, iExact) {
                    if (/^~/.test(iFilter)) {
                        var indx, patternIndx = 0, len = iExact.length, pattern = iFilter.slice(1);
            ...
            setDefaults: function(table, c, wo) {
                var indx, filters = [], columns = c.columns;
            ...
            buildRow: function(table, c, wo) {
                var column, $header, buildSelect, disabled, // c.columns defined in computeThIndexes()
                columns = c.columns, buildFilter = '<tr class="tablesorter-filter-row">';
                ...
                for (column = 0; column < columns; column++) {
                    ...
                    if (buildFilter) {
                        ...
                        if (disabled) {
                            buildFilter.addClass("disabled")[0].disabled = true;
                        }
                    }
                }
            },
            ...
            checkFilters: function(table, filter) {
                var c = table.config, wo = c.widgetOptions, filterArray = $.isArray(filter), filters = filterArray ? filter : ts.getFilters(table), combinedFilters = (filters || []).join("");
            ...
            findRows: function(table, filters, combinedFilters) {
                var cached, len, $rows, rowIndex, tbodyIndex, $tbody, $cells, columnIndex, childRow, childRowText, exact, iExact, iFilter, lastSearch, matches, result, searchFiltered, filterMatched, showRow, time, c = table.config, wo = c.widgetOptions, columns = c.columns, $tbodies = c.$tbodies, // anyMatch really screws up with these types of filters
                anyMatchNotAllowedTypes = [ "range", "operators" ], // parse columns after formatter, in case the class is added at that point
                        ...
                        for (rowIndex = 0; rowIndex < len; rowIndex++) {
                            ...
                            for (columnIndex = 0; columnIndex < columns; columnIndex++) {
                                ...
                                if (filters[columnIndex] || wo.filter_anyMatch) {
                                    ...
                                    result = showRow;
                                    // if showRow is true, show that row
                            ...
                            if (childRow.length) {
                                if (c.pager && c.pager.countChildRows || wo.pager_countChildRows) {
                                    childRow[showRow ? "removeClass" : "addClass"](wo.filter_filteredRow);
                                }
            ...
            buildSelect: function(table, column, updating, onlyavail) {
                ...
                var indx, rowIndex, tbodyIndex, len, currentValue, txt, c = table.config, wo = c.widgetOptions, $tbodies = c.$tbodies, arry = [], node = c.$headers.filter('[data-column="' + column + '"]:last'), // t.data('placeholder') won't work in jQuery older than 1.4.3
            ...
            buildDefault: function(table, updating) {
                var columnIndex, $header, c = table.config, wo = c.widgetOptions, columns = c.columns;
                ...
                for (columnIndex = 0; columnIndex < columns; columnIndex++) {
                    ...
                    if (($header.hasClass("filter-select") || wo.filter_functions && wo.filter_functions[columnIndex] === true) && !$header.hasClass("filter-false")) {
                        ...
                        wo.filter_functions[columnIndex] = true;
        ...
        ts.setFilters = function(table, filter, apply) {
            var $table = $(table), c = $table.length ? $table[0].config : {}, valid = c && c.$filters ? c.$filters.each(function(indx, el) {
        ...
        ts.addWidget({
            ...
            format: function(table, c, wo) {
                ...
                var $t = c.$table, $win = $(window), header = $t.children("thead:first"), hdrCells = header.children("tr:not(.sticky-false)").children(), innr = ".tablesorter-header-inner", tfoot = $t.find("tfoot"), filterInputs = ".tablesorter-filter", $stickyOffset = isNaN(wo.stickyHeaders_offset) ? $(wo.stickyHeaders_offset) : "", stickyOffset = $stickyOffset.length ? $stickyOffset.height() || 0 : parseInt(wo.stickyHeaders_offset, 10) || 0, stickyzIndex = wo.stickyHeaders_zIndex ? wo.stickyHeaders_zIndex : 2, $stickyTable = wo.$sticky = $t.clone().addClass("containsStickyHeaders").css({
                    position: "fixed",
                    margin: 0,
                    top: stickyOffset,
                    visibility: "hidden",
                    zIndex: stickyzIndex
                }), stkyHdr = $stickyTable.children("thead:first").addClass("tablesorter-stickyHeader " + wo.stickyHeaders), stkyCells, laststate = "", spacing = 0, flag = false, resizeHdr = function() {
                ...
                $t.addClass("hasStickyHeaders").bind("sortEnd.tsSticky", function() {
                    hdrCells.filter(":visible").each(function(i) {
                        var t = stkyCells.filter(":visible").eq(i);
                        t.attr("class", $(this).attr("class")).removeClass(ts.css.processing + " " + c.cssProcessing);
                        if (c.cssIcon) {
                            t.find("." + ts.css.icon).attr("class", $(this).find("." + ts.css.icon).attr("class"));
                        }
                    });
                ...
                hdrCells.find(c.selectorSort).add(c.$headers.filter(c.selectorSort)).each(function(i) {
                    var t = $(this), // clicking on sticky will trigger sort
                ...
                $win.bind("scroll.tsSticky resize.tsSticky", function(e) {
                    ...
                    // fixes #278
                    var pre = "tablesorter-sticky-", offset = $t.offset(), cap = wo.stickyHeaders_includeCaption ? 0 : $t.find("caption").outerHeight(true), sTop = $win.scrollTop() + stickyOffset - cap, tableHt = $t.height() - ($stickyTable.height() + (tfoot.height() || 0)), vis = sTop > offset.top && sTop < offset.top + tableHt ? "visible" : "hidden";
                    $stickyTable.removeClass(pre + "visible " + pre + "hidden").addClass(pre + vis).css({
                ...
                stkyCells.find(filterInputs).bind("keyup search change", function(e) {
                    ...
                    c.$filters.find(filterInputs).eq(col).val($f.val()).trigger("search");
                    setTimeout(function() {
                        flag = false;
        ...
        ts.addWidget({
            ...
            format: function(table, c, wo) {
                ...
                var $t, t, i, j, s = {}, $c, $cols, w, tw, $tbl = c.$table, position = 0, $target = null, $next = null, fullWidth = Math.abs($tbl.parent().width() - $tbl.width()) < 20, stopResize = function() {
                ...
                $cols.each(function() {
                    $t = $(this);
                    j = parseInt($t.css("padding-right"), 10) + 10;
                    // 8 is 1/2 of the 16px wide resizer grip
                    t = '<div class="tablesorter-resizer" style="cursor:w-resize;position:absolute;z-index:1;right:-' + j + 'px;top:0;height:100%;width:20px;"></div>';
                    $t.find(".tablesorter-wrapper").append(t);
                    ...
                    stopResize();
                }).find(".tablesorter-resizer,.tablesorter-resizer-grip").bind("mousedown", function(e) {
                ...
                $tbl.find("thead:first").bind("mouseup.tsresize mouseleave.tsresize", function() {
                    stopResize();
                }).bind("contextmenu.tsresize", function() {
                    ts.resizableReset(table);
                    // $.isEmptyObject() needs jQuery 1.4+
                    var rtn = $.isEmptyObject ? $.isEmptyObject(s) : s === {};
                    // allow right click if already reset
                    s = {};
                    return rtn;
                });
            },
        ...
        ts.addWidget({
            ...
            format: function(table, c, wo, init) {
                var sl, time, $t = c.$table, ss = wo.saveSort !== false, // make saveSort active/inactive; default to true
                sortList = {
                    sortList: c.sortList
                };
                if (c.debug) {
