define("gallery/tablesorter/2.13.3/widgets-filter-formatter-debug", [ "$-debug" ], function(require, exports, module) {
	...
	\**********************/
            uiSpinner: function($cell, indx, spinnerDef) {
                var o = $.extend({
                    ...
                    exactMatch: true,
                    compare: ""
                }, spinnerDef), // Add a hidden input to hold the range values
                $input = $('<input class="filter" type="hidden">').appendTo($cell).bind("change.tsfilter", function() {
                    updateSpinner({
                        value: this.value,
                        delayed: false
                    });
                }), $shcell = [], c = $cell.closest("table")[0].config, // this function updates the hidden input and adds the current values to the header cell text
                updateSpinner = function(ui) {
                    var chkd = true, state, // ui is not undefined on create
                    ...
                    $cell.find(".filter").val(chkd ? (o.compare ? o.compare : o.exactMatch ? "=" : "") + v : "").trigger("search", ui && typeof ui.delayed === "boolean" ? ui.delayed : o.delayed).end().find(".spinner").spinner(state).val(v);
                ...
                o.create = function(event, ui) {
                    updateSpinner();
                    // ui is an empty object on create
                ...
                if (o.addToggle) {
                    $('<div class="button"><input id="uispinnerbutton' + indx + '" type="checkbox" class="toggle" /><label for="uispinnerbutton' + indx + '"></label></div>').appendTo($cell).find(".toggle").bind("change", function() {
                        updateSpinner();
                    });
                }
                ...
                $('<input class="spinner spinner' + indx + '" />').val(o.value).appendTo($cell).spinner(o).bind("change keyup", function() {
                    updateSpinner();
                });
                // has sticky headers?
                c.$table.bind("stickyHeadersInit", function() {
                    ...
                    if (o.addToggle) {
                        $('<div class="button"><input id="stickyuispinnerbutton' + indx + '" type="checkbox" class="toggle" /><label for="stickyuispinnerbutton' + indx + '"></label></div>').appendTo($shcell).find(".toggle").bind("change", function() {
                            $cell.find(".toggle")[0].checked = this.checked;
                            updateSpinner();
                        });
                    }
                    // add a jQuery UI spinner!
                    $('<input class="spinner spinner' + indx + '" />').val(o.value).appendTo($shcell).spinner(o).bind("change keyup", function() {
	...
	\**********************/
            uiSlider: function($cell, indx, sliderDef) {
                var o = $.extend({
                    ...
                    compare: "",
                    allText: "all"
                }, sliderDef), // Add a hidden input to hold the range values
                $input = $('<input class="filter" type="hidden">').appendTo($cell).bind("change.tsfilter", function() {
                    updateSlider({
                        value: this.value
                    });
                ...
                updateSlider = function(ui) {
                    ...
                    var v = typeof ui !== "undefined" ? $.tablesorter.formatFloat((ui.value + "").replace(/[><=]/g, "")) || o.min : o.value, val = o.compare ? v : v === o.min ? o.allText : v, result = o.compare + val;
                    if (o.valueToHeader) {
                    ...
                    $cell.find(".filter").val(o.compare ? o.compare + v : v === o.min ? "" : (o.exactMatch ? "=" : "") + v).trigger("search", ui && typeof ui.delayed === "boolean" ? ui.delayed : o.delayed).end().find(".slider").slider("value", v);
                    // update sticky header cell
                    if ($shcell.length) {
                ...
                $('<div class="slider slider' + indx + '"/>').appendTo($cell).slider(o);
                // on reset
                c.$table.bind("filterReset", function() {
                ...
                c.$table.bind("stickyHeadersInit", function() {
                    $shcell = c.widgetOptions.$sticky.find(".tablesorter-filter-row").children().eq(indx).empty();
                    // add a jQuery UI slider!
                    $('<div class="slider slider' + indx + '"/>').val(o.value).appendTo($shcell).slider(o).bind("change keyup", function() {
	...
	\*************************/
            uiRange: function($cell, indx, rangeDef) {
                var o = $.extend({
                    ...
                    delayed: true,
                    valueToHeader: false
                }, rangeDef), // Add a hidden input to hold the range values
                $input = $('<input class="filter" type="hidden">').appendTo($cell).bind("change.tsfilter", function() {
                ...
                updateUiRange = function(ui) {
                    ...
                    var val = ui && ui.values || o.values, result = val[0] + " - " + val[1], // make range an empty string if entire range is covered so the filter row will hide (if set)
                    ...
                    if (o.valueToHeader) {
                        ...
                        // add values to the handle data-value attribute so the css tooltip will work properly
                        $cell.find(".ui-slider-handle").addClass("value-popup").eq(0).attr("data-value", val[0]).end().eq(1).attr("data-value", val[1]);
                    }
                    // update the hidden input
                    $cell.find(".filter").val(range).trigger("search", ui && typeof ui.delayed === "boolean" ? ui.delayed : o.delayed).end().find(".range").slider("values", val);
                    // update sticky header cell
                    if ($shcell.length) {
                        ...
                        if (o.valueToHeader) {
                            ...
                            $shcell.find(".ui-slider-handle").addClass("value-popup").eq(0).attr("data-value", val[0]).end().eq(1).attr("data-value", val[1]);
                        }
                    }
                };
                $cell.closest("thead").find("th[data-column=" + indx + "]").addClass("filter-parsed");
                ...
                $('<div class="range range' + indx + '"/>').appendTo($cell).slider(o);
                // on reset
                c.$table.bind("filterReset", function() {
                ...
                c.$table.bind("stickyHeadersInit", function() {
                    $shcell = c.widgetOptions.$sticky.find(".tablesorter-filter-row").children().eq(indx).empty();
                    // add a jQuery UI slider!
                    $('<div class="range range' + indx + '"/>').val(o.value).appendTo($shcell).slider(o).bind("change keyup", function() {
	...
	\*************************/
            uiDateCompare: function($cell, indx, defDate) {
                ...
                }, defDate), $hdr = $cell.closest("thead").find("th[data-column=" + indx + "]"), // Add a hidden input to hold the range values
                $input = $('<input class="dateCompare" type="hidden">').appendTo($cell).bind("change.tsfilter", function() {
                    var v = this.value;
                    if (v) {
                        o.onClose(v);
                    }
                }), t, l, $shcell = [], c = $cell.closest("table")[0].config, // this function updates the hidden input
                updateCompare = function(v) {
                    var date = new Date($cell.find(".date").datepicker("getDate")).getTime();
                    $cell.find(".compare").val(v);
                    $cell.find(".dateCompare").val(v + date).trigger("search", o.delayed).end();
                    // update sticky header cell
                    if ($shcell.length) {
                ...
                if (o.compareOptions) {
                    ...
                    for (var myOption in o.compareOptions) {
                        ...
                        if (myOption === o.compare) l += ' selected="selected"';
                        l += ">" + myOption + "</option>";
                    }
                    l += "</select>";
                    $cell.append(l).find(".compare").bind("change", function() {
                ...
                t = '<input type="text" class="date date' + indx + '" placeholder="' + ($hdr.data("placeholder") || $hdr.attr("data-placeholder") || "") + '" />';
                $(t).appendTo($cell);
                // add callbacks; preserve added callbacks
                ...
                o.onClose = function(selectedDate, ui) {
                    ...
                    $cell.find(".dateCompare").val(compare + date).trigger("search").end().find(".date").datepicker("setDate", selectedDate);
                    // update sticky header cell
                    if ($shcell.length) {
                ...
                c.$table.bind("stickyHeadersInit", function() {
                    $shcell = c.widgetOptions.$sticky.find(".tablesorter-filter-row").children().eq(indx).empty();
                    if (o.compareOptions) {
                        $shcell.append(l).find(".compare").bind("change", function() {
                            updateCompare($(this).val());
                        });
                    } else if (o.cellText) {
                        $shcell.append(l);
                    ...
                    $shcell.append(t).find(".date").datepicker(o);
                });
                // return the hidden input so the filter widget has a reference to it
	...
	\*************************/
            uiDatepicker: function($cell, indx, defDate) {
                var o = $.extend({
                    ...
                    changeYear: true,
                    numberOfMonths: 1
                }, defDate), t, closeTo, closeFrom, $shcell = [], // Add a hidden input to hold the range values
                $input = $('<input class="dateRange" type="hidden">').appendTo($cell).bind("change.tsfilter", function() {
                    var v = this.value;
                    if (v.match(" - ")) {
                        v = v.split(" - ");
                ...
                closeFrom = o.onClose = function(selectedDate, ui) {
                    var from = new Date($cell.find(".dateFrom").datepicker("getDate")).getTime() || "", to = new Date($cell.find(".dateTo").datepicker("getDate")).getTime() || "", range = from ? to ? from + " - " + to : ">=" + from : to ? "<=" + to : "";
                    $cell.find(".dateRange").val(range).trigger("search").end().find(".dateTo").datepicker("option", "minDate", selectedDate).end().find(".dateFrom").val(selectedDate);
                    // update sticky header cell
                    if ($shcell.length) {
                        $shcell.find(".dateTo").datepicker("option", "minDate", selectedDate).end().find(".dateFrom").val(selectedDate);
                    }
                    if (typeof o.oldonClose === "function") {
                        o.oldonClose(selectedDate, ui);
                    }
                ...
                closeTo = o.onClose = function(selectedDate, ui) {
                    var from = new Date($cell.find(".dateFrom").datepicker("getDate")).getTime() || "", to = new Date($cell.find(".dateTo").datepicker("getDate")).getTime() || "", range = from ? to ? from + " - " + to : ">=" + from : to ? "<=" + to : "";
                    $cell.find(".dateRange").val(range).trigger("search").end().find(".dateFrom").datepicker("option", "maxDate", selectedDate).end().find(".dateTo").val(selectedDate);
                    // update sticky header cell
                    if ($shcell.length) {
                        $shcell.find(".dateFrom").datepicker("option", "maxDate", selectedDate).end().find(".dateTo").val(selectedDate);
                    }
                    if (typeof o.oldonClose === "function") {
                        o.oldonClose(selectedDate, ui);
                    }
	...
	\**********************/
            html5Number: function($cell, indx, def5Num) {
                var t, o = $.extend({
                    ...
                    compareOptions: false,
                    skipTest: false
                }, def5Num), // test browser for HTML5 range support
                $number = $('<input type="number" style="visibility:hidden;" value="test">').appendTo($cell), // test if HTML5 number is supported - from Modernizr
                numberSupported = o.skipTest || $number.attr("type") === "number" && $number.val() !== "test", l, $shcell = [], c = $cell.closest("table")[0].config, updateCompare = function(v) {
                    var number = $cell.find(".number").val();
                    $cell.find(".compare").val(v);
                    $cell.find("input[type=hidden]").val(v + number).trigger("search", o.delayed).end();
                    // update sticky header cell
                    if ($shcell.length) {
                        $shcell.find(".compare").val(v);
                    }
                    ...
                    var compare = $cell.find(".compare").val() || o.compare;
                    $cell.find("input[type=hidden]").val(!o.addToggle || chkd ? (compare ? compare : o.exactMatch ? "=" : "") + v : "").trigger("search", delayed ? delayed : o.delayed).end().find(".number").val(v);
                ...
                if (numberSupported) {
                    t = '<input class="number" type="number" min="' + o.min + '" max="' + o.max + '" value="' + o.value + '" step="' + o.step + '" />';
                    // add HTML5 number (spinner)
                    $cell.append(t + '<input type="hidden" />').find(".toggle, .number").bind("change", function() {
                        updateNumber($cell.find(".number").val());
                    }).closest("thead").find("th[data-column=" + indx + "]").addClass("filter-parsed").closest("table").bind("filterReset", function() {
                        // turn off the toggle checkbox
                        if (o.addToggle) {
                            $cell.find(".toggle")[0].checked = false;
                            if ($shcell.length) {
                    ...
                    c.$table.bind("stickyHeadersInit", function() {
                        ...
                        $shcell.append(t).find(".toggle, .number").bind("change", function() {
	...
	\**********************/
            html5Range: function($cell, indx, def5Range) {
                var o = $.extend({
                    ...
                    allText: "all",
                    skipTest: false
                }, def5Range), // test browser for HTML5 range support
                $range = $('<input type="range" style="visibility:hidden;" value="test">').appendTo($cell), // test if HTML5 range is supported - from Modernizr (but I left out the method to detect in Safari 2-4)
                // see https://github.com/Modernizr/Modernizr/blob/master/feature-detects/inputtypes.js
                rangeSupported = o.skipTest || $range.attr("type") === "range" && $range.val() !== "test", $shcell = [], c = $cell.closest("table")[0].config, updateRange = function(v, delayed) {
                    /*jshint eqeqeq:false */
                    v = (v + "").replace(/[<>=]/g, "") || o.min;
                    // hidden input changes may include compare symbols
                    var t = " (" + (o.compare ? o.compare + v : v == o.min ? o.allText : v) + ")";
                    $cell.find("input[type=hidden]").val(o.compare ? o.compare + v : v == o.min ? "" : (o.exactMatch ? "=" : "") + v).trigger("search", delayed ? delayed : o.delayed).end().find(".range").val(v);
                ...
                if (rangeSupported) {
                    // add HTML5 range
                    $cell.html('<input type="hidden"><input class="range" type="range" min="' + o.min + '" max="' + o.max + '" value="' + o.value + '" />').closest("thead").find("th[data-column=" + indx + "]").addClass("filter-parsed").find(".tablesorter-header-inner").append('<span class="curvalue" />');
                    ...
                    c.$table.bind("stickyHeadersInit", function() {
                        $shcell = c.widgetOptions.$sticky.find(".tablesorter-filter-row").children().eq(indx).empty();
                        $shcell.html('<input class="range" type="range" min="' + o.min + '" max="' + o.max + '" value="' + o.value + '" />').find(".range").bind("change", function() {
	...
	\**********************/
            html5Color: function($cell, indx, defColor) {
                var t, o = $.extend({
                    ...
                    valueToHeader: false,
                    skipTest: false
                }, defColor), // Add a hidden input to hold the range values
                $color = $('<input type="color" style="visibility:hidden;" value="test">').appendTo($cell), // test if HTML5 color is supported - from Modernizr
                colorSupported = o.skipTest || $color.attr("type") === "color" && $color.val() !== "test", $shcell = [], c = $cell.closest("table")[0].config, updateColor = function(v) {
                    v = v || o.value;
                    var chkd = true, t = " (" + v + ")";
                    if (o.addToggle) {
                        chkd = $cell.find(".toggle").is(":checked");
                    ...
                    $cell.find("input[type=hidden]").val(chkd ? v + (o.exactMatch ? "=" : "") : "").trigger("search");
                    if (o.valueToHeader) {
                ...
                if (colorSupported) {
                    ...
                    c.$table.bind("stickyHeadersInit", function() {
                        ...
                        $shcell.html(t).find(".toggle, .colorpicker").bind("change", function() {
