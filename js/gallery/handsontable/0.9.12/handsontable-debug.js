define("gallery/handsontable/0.9.12/handsontable-debug", [ "$-debug" ], function(require, exports, module) {
 ...
 */
        Handsontable.Core = function(rootElement, userSettings) {
            var priv, datamap, grid, selection, editproxy, autofill, instance = this, GridSettings = function() {};
            Handsontable.helper.inherit(GridSettings, DefaultSettings);
            //create grid settings as a copy of default settings
            Handsontable.helper.extend(GridSettings.prototype, Handsontable.TextCell);
            //overwrite defaults with default cell
            ...
            if (!this.rootElement[0].id) {
                this.rootElement[0].id = this.guid;
            }
            priv = {
                cellSettings: [],
                columnSettings: [],
     ...
     */
                createRow: function(index) {
                    var row, rowCount = instance.countRows();
     ...
     */
                createCol: function(index) {
                    ...
                    var r = 0, rlen = instance.countRows(), data = GridSettings.prototype.data, constructor = Handsontable.helper.columnFactory(GridSettings, priv.columnsSettingConflicts);
     ...
     */
                spliceCol: function(col, index, amount) {
     ...
     * param {...*} elements Optional. The elements to add to the array. If you don't specify any elements, spliceCol simply removes elements from the array
     */
                spliceRow: function(row, index, amount) {
                    var elements = 4 <= arguments.length ? [].slice.call(arguments, 3) : [];
     ...
     */
         ...
         *    }]}
         */
                        return datamap.getVars.prop(priv.settings.data.slice(datamap.getVars.row, datamap.getVars.row + 1)[0]);
                    } else {
     ...
     */
                ...
                set: function(row, prop, value, source) {
                    ...
                    if (typeof datamap.setVars.prop === "string" && datamap.setVars.prop.indexOf(".") > -1) {
                        ...
                        datamap.setVars.prop(priv.settings.data.slice(datamap.setVars.row, datamap.setVars.row + 1)[0], datamap.setVars.value);
                    } else {
     ...
     */
                physicalRowsToLogical: function(index, amount) {
                    ...
                    while (physicRow < GridSettings.prototype.data.length && rowsToRemove) {
                        this.get(physicRow, 0);
                        //this performs an actual mapping and saves the result to getVars
     ...
     */
            ...
            this.selection = selection = {
                //this public assignment is only temporary
                inProgress: false,
                /**
     ...
     */
                deselect: function() {
                    ...
                    instance.selection.inProgress = false;
                    //needed by HT inception
                    priv.selEnd = new Handsontable.SelectionPoint();
                    //create new empty point to remove the existing one
                    instance.view.wt.selections.current.clear();
                    instance.view.wt.selections.area.clear();
                    editproxy.destroy();
                    selection.refreshBorders();
     ...
     */
                init: function() {
                    ...
                    priv.onPaste = function onPaste(str) {
                        ...
                        var input = str.replace(/^[\r\n]*/g, "").replace(/[\r\n]*$/g, ""), inputArray = SheetClip.parse(input), coords = grid.getCornerCoords([ priv.selStart.coords(), priv.selEnd.coords() ]), areaStart = coords.TL, areaEnd = {
                    ...
                    function onKeyDown(event) {
                        ...
                        if (Array.prototype.filter.call(document.body.querySelectorAll(".context-menu-list"), instance.view.wt.wtDom.isVisible).length) {
                            //faster than $body.children('.context-menu-list:visible').length
                        ...
                        if (selection.isSelected()) {
                            ...
                            if (Handsontable.helper.isPrintableChar(event.keyCode) && ctrlDown) {
                                ...
                                } else if (event.keyCode === 89 || event.shiftKey && event.keyCode === 90) {
     ...
     */
                destroy: function(revertOriginal) {
                    if (typeof priv.editorDestroyer === "function") {
                        var destroyer = priv.editorDestroyer;
                        //this copy is needed, otherwise destroyer can enter an infinite loop
     ...
     */
                setCopyableText: function() {
                    ...
                    instance.copyPaste.copyable(datamap.getText({
                        row: startRow,
                        col: startCol
                    }, {
                        row: finalEndRow,
                    ...
                    if (changes[i] === null) {
                        ...
                        if (cellProperties.dataType === "number" && typeof changes[i][3] === "string") {
                            if (changes[i][3].length > 0 && /^-?[\d\s]*\.?\d*$/.test(changes[i][3])) {
                                changes[i][3] = numeral().unformat(changes[i][3] || "0");
                            }
                        }
   ...
   */
            this.setDataAtCell = function(row, col, value, source) {
                var input = setDataInputToArray(row, col, value), i, ilen, changes = [], prop;
   ...
   */
            this.setDataAtRowProp = function(row, prop, value, source) {
                var input = setDataInputToArray(row, prop, value), i, ilen, changes = [];
                for (i = 0, ilen = input.length; i < ilen; i++) {
   ...
   */
            this.populateFromArray = function(row, col, input, endRow, endCol, source, method) {
                if (typeof input !== "object") {
                    throw new Error("populateFromArray parameter `input` must be an array");
                }
   ...
   */
            this.getSelected = function() {
                //https://github.com/warpech/jquery-handsontable/issues/44  //cjl
                if (selection.isSelected()) {
   ...
   */
            this.getCellMeta = function(row, col) {
                var prop = datamap.colToProp(col), cellProperties;
     ...
     */
                function translateColIndex(col) {
                    return Handsontable.PluginHooks.execute(instance, "modifyCol", col);
                }
            };
            /**
   ...
   */
            this.countEmptyRows = function(ending) {
                var i = instance.countRows() - 1, empty = 0;
                while (i >= 0) {
                    datamap.get(i, 0);
   ...
   */
            this.countEmptyCols = function(ending) {
                ...
                var i = instance.countCols() - 1, empty = 0;
                while (i >= 0) {
   ...
   */
            ...
            (function() {
                ...
                var _run = instance.PluginHooks.run, _exe = instance.PluginHooks.execute;
   ...
   */
        ...
        $.fn.handsontable = function(action) {
            var i, ilen, args, output, userSettings, $this = this.first(), instance = $this.data("handsontable");
            // Init case
 ...
 */
        Handsontable.TableView = function(instance) {
            var that = this, $window = $(window), $documentElement = $(document.documentElement);
            this.instance = instance;
            this.settings = instance.getSettings();
            this.settingsFromDOM = instance.getSettingsFromDOM();
            instance.rootElement.data("originalStyle", instance.rootElement[0].getAttribute("style"));
            //needed to retrieve original style in jsFiddle link generator in HT examples. may be removed in future versions
            ...
            $documentElement.on("mousedown." + instance.guid, function(event) {
                ...
                if (next !== that.wt.wtTable.spreader) {
            ...
            instance.$table.on("mouseenter", function() {
                if (dragInterval) {
                    //if dragInterval was set (that means mouse was really outside of table, not over an element that is outside of <table> in DOM
                    clearInterval(dragInterval);
                    dragInterval = null;
                }
            });
            instance.$table.on("mouseleave", function(event) {
                ...
                if (event.pageY < offsetTop) {
                    //top edge crossed
                    row = -1;
                    method = "scrollVertical";
                } else if (event.pageY >= offsetTop + height) {
                    //bottom edge crossed
                    row = 1;
                    method = "scrollVertical";
                ...
                } else if (event.pageX >= offsetLeft + width) {
                    //right edge crossed
                    col = 1;
                    method = "scrollHorizontal";
                }
            ...
            var walkontableConfig = {
                ...
                onCellMouseDown: function(event, coords, TD) {
                    ...
                    if (event.button === 2 && instance.selection.inInSelection(coordsObj)) {} else if (event.shiftKey) {
     ...
     clearTextSelection(); //otherwise text selection blinks during multiple cells selection
                ...
                onCellMouseOver: function(event, coords) {
                    var coordsObj = {
                        row: coords[0],
                        col: coords[1]
                    };
         ...
         clearTextSelection(); //otherwise text selection blinks during multiple cells selection
            ...
            $(that.wt.wtTable.spreader).on("mousedown.handsontable, contextmenu.handsontable", function(event) {
                if (event.target === that.wt.wtTable.spreader && event.which === 3) {
                    //right mouse button exactly on spreader means right clickon the right hand side of vertical scrollbar
                    event.stopPropagation();
                }
            });
        ...
        Handsontable.TableView.prototype.applyCellTypeMethod = function(methodName, td, row, col) {
            var prop = this.instance.colToProp(col), cellProperties = this.instance.getCellMeta(row, col), method = Handsontable.helper.getCellMethod(methodName, cellProperties[methodName]);
            //methodName is 'renderer' or 'editor'
 ...
 */
        Handsontable.UndoRedo.prototype.add = function(changes, source) {
            ...
            //if we are in point abcdef(g)hijk in history, remove everything after (g)
 ...
 */
        Handsontable.TextRenderer = function(instance, TD, row, col, prop, value, cellProperties) {
            ...
            instance.view.wt.wtDom.fastInnerText(TD, escaped);
 ...
 */
        Handsontable.AutocompleteRenderer = function(instance, TD, row, col, prop, value, cellProperties) {
            var TEXT = clonableTEXT.cloneNode(false);
 ...
 */
        Handsontable.CheckboxRenderer = function(instance, TD, row, col, prop, value, cellProperties) {
            ...
            if (value === cellProperties.checkedTemplate || value === Handsontable.helper.stringify(cellProperties.checkedTemplate)) {
                ...
                instance.view.wt.wtDom.fastInnerText(TD, "#bad value#");
            }
            var $input = $(INPUT);
            if (cellProperties.readOnly) {
 ...
 */
        ...
        HandsontableTextEditorClass.prototype.createElements = function() {
            this.STATE_VIRGIN = "STATE_VIRGIN";
            //before editing
            ...
            this.$textarea.off(".editor").on("keydown.editor", function(event) {
                ...
                switch (event.keyCode) {
                  ...
                  case 13:
                    ...
                    if (event.ctrlKey && !isMultipleSelection || event.altKey) {
                  ...
                  default:
                    event.stopImmediatePropagation();
                    //backspace, delete, home, end, CTRL+A, CTRL+C, CTRL+V, CTRL+X should only work locally when cell is edited (not in table context)
                    break;
                }
        ...
        HandsontableTextEditorClass.prototype.bindTemporaryEvents = function(td, row, col, prop, value, cellProperties) {
            ...
            this.beforeKeyDownHook = function(event) {
                ...
                if (Handsontable.helper.isPrintableChar(event.keyCode)) {
                    ...
                    event.stopImmediatePropagation();
                    event.preventDefault();
                } else if (event.keyCode === 13 && that.instance.getSettings().enterBeginsEditing) {
                    //enter
                    var selected = that.instance.getSelected();
                    var isMultipleSelection = !(selected[0] === selected[2] && selected[1] === selected[3]);
                    ...
                    event.preventDefault();
                    //prevent new line at the end of textarea
                    event.stopImmediatePropagation();
                }
            };
 ...
 */
        ...
        HandsontableTextEditorClass.prototype.beginEditing = function(row, col, prop, useOriginalValue, suffix) {
            ...
            //need it instantly, to prevent https://github.com/warpech/jquery-handsontable/issues/348
            this.TEXTAREA.focus();
        ...
        HandsontableTextEditorClass.prototype.refreshDimensions = function() {
            ...
            var $td = $(this.TD);
            //because old td may have been scrolled out with scrollViewport
            ...
            var width = $td.width(), maxWidth = this.instance.view.maximumVisibleElementWidth(editLeft) - 10, height = $td.outerHeight() - 4, maxHeight = this.instance.view.maximumVisibleElementHeight(editTop) - 5;
        ...
        HandsontableTextEditorClass.prototype.discardEditor = function(result) {
            ...
            if (result === false && this.cellProperties.allowInvalid !== true) {
                ...
                if (this.waitingEvent) {
                    //this is needed so when you finish editing with Enter key, the default Enter behavior (move selection down) will work after async validation
            ...
            this.$textarea.off(".acEditor").on("keydown.acEditor", function(event) {
                switch (event.keyCode) {
                  case 38:
                    ...
                    that.typeahead.prev();
                    event.stopImmediatePropagation();
                    //stops TextEditor and core onKeyDown handler
                    break;
 ...
 */
        HandsontableAutocompleteEditorClass.prototype.bindTemporaryEvents = function(td, row, col, prop, value, cellProperties) {
            ...
            this.typeahead.select = function() {
                var output = this.hide();
                //need to hide it before destroyEditor, because destroyEditor checks if menu is expanded
                that.instance.destroyEditor(true);
            ...
            function onDblClick() {
                that.beginEditing(row, col, prop, true);
                that.instance.registerTimeout("IE9_align_fix", function() {
                    //otherwise is misaligned in IE9
                    that.typeahead.lookup();
                }, 1);
            }
            this.instance.view.wt.update("onCellDblClick", onDblClick);
 ...
 */
        ...
        HandsontableHandsontableEditorClass.prototype.finishEditing = function(isCancelled, ctrlDown) {
            if (this.$htContainer.handsontable("isListening")) {
 ...
 */
        ...
        Handsontable.PluginHookClass = function() {
            ...
            PluginHookClass.prototype.remove = function(key, fn) {
                var status = false, hookTypes = [ "persistent", "once" ], type, x, lenx, i, leni;
                // provide support for old versions of HOT
                if (key in legacy) {
                    key = legacy[key];
                }
            ...
            PluginHookClass.prototype.run = function(instance, key, p1, p2, p3, p4, p5) {
                var hookTypes = [ "persistent", "once" ], type, x, lenx, i, leni;
                // provide support for old versions of HOT
                if (key in legacy) {
                    key = legacy[key];
                }
            ...
            PluginHookClass.prototype.execute = function(instance, key, p1, p2, p3, p4, p5) {
                var hookTypes = [ "persistent", "once" ], type, x, lenx, i, leni, res;
                // provide support for old versions of HOT
                if (key in legacy) {
                    key = legacy[key];
                }
        ...
        (function(Handsontable) {
            function HandsontableAutoColumnSize() {
                ...
                this.determineColumnWidth = function(col) {
                    var instance = this, tmp = instance.autoColumnSizeTmp, d;
                    if (!tmp.container) {
                    ...
                    if (cellProperties.renderer) {
                        ...
                        width += instance.view.wt.wtDom.outerWidth(tmp.renderer) - instance.view.wt.wtDom.outerWidth(tmp.noRenderer);
                    }
                    var maxWidth = instance.view.wt.wtViewport.getViewportWidth() - 2;
                    //2 is some overhead for cell border
                    if (width > maxWidth) {
                        width = maxWidth;
                    }
 ...
 */
        function HandsontableColumnSorting() {
            ...
            this.sort = function() {
                ...
                //this is required by translateRow plugin hook
        ...
        (function(Handsontable) {
            ...
            function createContextMenu() {
                var instance = this, selectorId = instance.rootElement[0].id, allItems = {
                    row_above: {
                ...
                }, options = {}, i, ilen, settings = instance.getSettings();
                function onContextClick(key) {
                    var corners = instance.getSelected();
       ...
       return true;
                ...
                } else if (Object.prototype.toString.apply(settings.contextMenu) === "[object Array]") {
                    //contextMenu is an array
                    options.items = {};
                ...
                } else if (Object.prototype.toString.apply(settings.contextMenu) === "[object Object]") {
                    //contextMenu is an options object as defined in http://medialize.github.com/jQuery-contextMenu/docs.html
 ...
 */
        ...
        function HandsontableManualColumnMove() {
            var instance, pressed, startCol, endCol, startX, startOffset;
        ...
        function HandsontableManualColumnResize() {
            var pressed, currentTH, currentCol, currentWidth, autoresizeTimeout, instance, newSize, startX, startWidth, startOffset, dblclick = 0, resizer = document.createElement("DIV"), handle = document.createElement("DIV"), line = document.createElement("DIV"), lineStyle = line.style;
            ...
            function refreshResizerPosition(TH) {
                instance = this;
                currentTH = TH;
                var col = this.view.wt.wtTable.getCoords(TH)[1];
                //getCoords returns array [row, col]
                if (col >= 0) {
                    //if not row header
                    currentCol = col;
            ...
            function getColumnWidth(TH) {
                ...
                var col = instance.view.wt.wtTable.getCoords(TH)[1];
                //getCoords returns array [row, col]
 ...
 * */
        ...
        (function(StorageClass) {
            function HandsontablePersistentState() {
                var plugin = this;
                this.init = function() {
                    var instance = this, pluginSettings = instance.getSettings()["persistentState"];
        ...
        (function($) {
            ...
            AutoResizer.prototype = {
                ...
                createClone: function() {
                    var el = this.el, self = this, config = this.config;
                    this.clones = $();
                    ...
                    this.clones.removeAttr("name").removeAttr("id").attr("tabIndex", -1).css(autoResize.cloneCSSValues).css("overflowY", "scroll");
                },
                check: function(e, immediate) {
                    var config = this.config, wClone = this.wClone, hClone = this.hClone, el = this.el, value = el.val();
                    if (wClone) {
                        wClone.text(value);
                        ...
                        if (newWidth < currentWidth && newWidth >= config.minWidth || newWidth >= config.minWidth && newWidth <= config.maxWidth) {
                            ...
                            el.scrollLeft(0);
                            config.animate && !immediate ? el.stop(1, 1).animate({
                                width: newWidth
                    ...
                    if (hClone) {
                        ...
                        hClone.height(0).val(value).scrollTop(1e4);
                ...
                injectClone: function() {
                    (autoResize.cloneContainer || (autoResize.cloneContainer = $("<arclones/>").appendTo("body"))).empty().append(this.clones);
                }
            };
        })(jQuery);
        /**
 ...
 */
  ...
  this.copyCallbacks.push(fn);
        ...
        CopyPasteClass.prototype._bindEvent = function() {
            if (window.jQuery) {
                //if jQuery exists, use jQuery event (for compatibility with $.trigger and $.triggerHandler, which can only trigger jQuery events - and we use that in tests)
            ...
            function observe(obj, callback) {
                ...
                if (Object.observe) {
                    ...
                    if (callback) {
                        ...
                        observer.next = null;
                        this.intervals = this.intervals || [ 100, 1e3, 1e4, 6e4 ];
 ...
 */
        ...
        WalkontableColumnStrategy.prototype.stretch = function() {
            ...
            var containerSize = this.getContainerSize(this.cellSizesSum), i = 0;
            this.remainingSize = this.cellSizesSum - containerSize;
            this.cellStretch.length = 0;
            //clear previous stretch
            if (this.strategy === "all") {
                if (this.remainingSize < 0) {
                    var ratio = containerSize / this.cellSizesSum;
                    var newSize;
                    while (i < this.cellCount - 1) {
                        //"i < this.cellCount - 1" is needed because last cellSize is adjusted after the loop
        ...
        function Walkontable(settings) {
            var that = this, originalHeaders = [];
            this.guid = "wt_" + (window.Handsontable ? Handsontable.helper.randomString() : "");
            //this is the namespace for global events
        ...
        Walkontable.prototype.draw = function(selectionsOnly) {
            ...
            if (this.drawn) {
                //fix offsets that might have changed
 ...
 * @return {void}
 */
        if (document.createTextNode("test").textContent) {
            //STANDARDS
 ...
 */
        WalkontableDom.prototype.isVisible = function(elem) {
            //fast method
            try {
                //try/catch performance is not a problem here: http://jsperf.com/try-catch-performance-overhead/7
 ...
 */
        WalkontableDom.prototype.offset = function(elem) {
            ...
            var offsetLeft = elem.offsetLeft, offsetTop = elem.offsetTop, lastElem = elem;
        ...
        (function() {
            ...
            function detectCaptionProblem() {
                ...
                hasCaptionProblem = TABLE.offsetHeight < 2 * TABLE.lastChild.offsetHeight;
                //boolean
 ...
 */
        ...
        WalkontableRowStrategy.prototype.add = function(i, TD, reverse) {
            if (this.remainingSize < 0) {
                ...
                this.cellCount++;
                this.remainingSize = this.cellSizesSum - containerSize;
                if (reverse && this.remainingSize > 0) {
                    //something is outside of the screen, maybe even some full rows?
                    while (this.cellCount > 0 && this.cellSizes[this.cellCount - 1] < this.remainingSize) {
                        //this row is completely off screen!
                        this.cellSizesSum -= this.cellSizes[this.cellCount - 1];
                        this.cellCount--;
                        this.cellSizes.length = this.cellCount;
                    }
                }
            }
        };
        ...
        WalkontableScroll.prototype.scrollVertical = function(delta) {
            ...
            var instance = this.instance, newOffset, offset = instance.getSetting("offsetRow"), fixedCount = instance.getSetting("fixedRowsTop"), total = instance.getSetting("totalRows"), maxSize = instance.wtViewport.getViewportHeight();
            if (total > 0) {
        ...
        WalkontableScroll.prototype.scrollHorizontal = function(delta) {
            ...
            var instance = this.instance, newOffset, offset = instance.getSetting("offsetColumn"), fixedCount = instance.getSetting("fixedColumnsLeft"), total = instance.getSetting("totalColumns"), maxSize = instance.wtViewport.getViewportWidth();
            if (total > 0) {
        ...
        WalkontableScroll.prototype.scrollLogicHorizontal = function(delta, offset, total, fixedCount, maxSize, cellSizeFn) {
            var newOffset = offset + delta, sum = 0, col;
            ...
            if (this.instance.drawn) {
                ...
                if (this.total > this.visibleCount) {
                    ...
                    if (delta === 1) {
                        ...
                        //is new offset different than old offset
                        if (this.type === "vertical") {
 ...
 */
        ...
        WalkontableScrollbar.prototype.refresh = function() {
            ...
            var ratio, sliderSize, handleSize, handlePosition, visibleCount = this.visibleCount, tableWidth = this.instance.wtViewport.getWorkspaceWidth(), tableHeight = this.instance.wtViewport.getWorkspaceHeight();
        ...
        WalkontableScrollbarNative.prototype.init = function() {
            ...
            //in future remove jQuery from here
            var that = this;
        ...
        WalkontableScrollbarNative.prototype.availableSize = function() {
            ...
            if (this.windowScrollPosition > this.tableParentOffset) {
                //last -1 means that viewport is scrolled behind the table
 ...
 */
        ...
        WalkontableSelection.prototype.draw = function() {
            ...
            var visibleRows = this.instance.wtTable.rowStrategy.countVisible(), visibleColumns = this.instance.wtTable.columnStrategy.countVisible();
        ...
        function WalkontableSettings(instance, settings) {
            ...
            this.defaults = {
                ...
                //presentation mode
                scrollH: "auto",
                //values: scroll (always show scrollbar), auto (show scrollbar if table does not fit in the container), none (never show scrollbar)
                scrollV: "auto",
                //values: see above
                scrollbarModelH: "dragdealer",
                //values: dragdealer, native
                scrollbarModelV: "dragdealer",
                //values: dragdealer, native
                stretchH: "hybrid",
                //values: hybrid, all, last, none
 ...
 var FLAG_NOT_VISIBLE_VERTICAL = 0x20; // 100000*/
        ...
        WalkontableTable.prototype.refreshHiderDimensions = function() {
            ...
            if (height !== Infinity || width !== Infinity) {
                ...
                if (height < 0) {
                    //this happens with WalkontableScrollbarNative and causes "Invalid argument" error in IE8
                    height = 0;
                }
        ...
        WalkontableTable.prototype.refreshStretching = function() {
            var instance = this.instance, stretchH = instance.getSetting("stretchH"), totalRows = instance.getSetting("totalRows"), totalColumns = instance.getSetting("totalColumns"), offsetColumn = instance.getSetting("offsetColumn");
        ...
        WalkontableTable.prototype.adjustAvailableNodes = function() {
            var displayTds, rowHeaders = this.instance.getSetting("rowHeaders"), displayThs = rowHeaders.length, columnHeaders = this.instance.getSetting("columnHeaders"), TR, TD, c;
            //adjust COLGROUP
            ...
            if (columnHeaders.length) {
                ...
                if (displayThs) {
                    TD = TR.firstChild;
                    //actually it is TH but let's reuse single variable
        ...
        WalkontableTable.prototype._doDraw = function() {
            var r = 0, source_r, c, source_c, offsetRow = this.instance.getSetting("offsetRow"), totalRows = this.instance.getSetting("totalRows"), totalColumns = this.instance.getSetting("totalColumns"), displayTds, rowHeaders = this.instance.getSetting("rowHeaders"), displayThs = rowHeaders.length, TR, TD, TH, adjusted = false, workspaceWidth, mustBeInViewport;
        ...
        WalkontableTable.prototype.refreshSelections = function(selectionsOnly) {
            var vr, r, vc, c, s, slen, classNames = [], visibleRows = this.rowStrategy.countVisible(), visibleColumns = this.columnStrategy.countVisible();
 ...
 */
        ...
        WalkontableTable.prototype.getCoords = function(TD) {
            return [ this.rowFilter.visibleToSource(this.wtDom.index(TD.parentNode)), this.columnFilter.visibleRowHeadedColumnToSourceColumn(TD.cellIndex) ];
        };
  ...
  //table is in viewport but how much exactly?
        ...
        WalkontableViewport.prototype.getViewportHeight = function(proposedHeight) {
            ...
            if (isNaN(this.columnHeaderHeight)) {
                var cellOffset = this.instance.wtDom.offset(this.instance.wtTable.TBODY), tableOffset = this.instance.wtTable.tableOffset;
        ...
        function WalkontableWheel(instance) {
            ...
            $(instance.wtTable.spreader).on("mousewheel", function(event, delta, deltaX, deltaY) {
                if (!deltaX && !deltaY && delta) {
                    //we are in IE8, see https://github.com/brandonaaron/jquery-mousewheel/issues/53
                    deltaY = delta;
                }
                if (!deltaX && !deltaY) {
                    //this happens in IE8 test case
                    return;
                }
                ...
                instance.wheelTimeout = setTimeout(function() {
                    //timeout is needed because with fast-wheel scrolling mousewheel event comes dozen times per second
                    if (deltaY) {
 ...
 */
        ...
        var Position = {
            get: function(obj) {
                ...
                //Walkontable patch. Original (var curleft = curtop = 0;) created curtop in global scope
                if (obj.offsetParent) {
                    do {
 ...
 */
        if (!Array.prototype.filter) {
            Array.prototype.filter = function(fun, thisp) {
                ...
                var len = thisp.length, res = [], i, val;
                for (i = 0; i < len; i += 1) {
 ...
 * ============================================================ */
   ...
   * ================== */
    ...
    (function() {
        ...
        Constants
    ************************************/
        var numeral, VERSION = "1.4.7", // internal storage for language config files
        languages = {}, currentLanguage = "en", zeroFormat = null, // check for nodeJS
        ...
        function unformatNumeral(n, string) {
            if (string.indexOf(":") > -1) {
                ...
                if (string === zeroFormat) {
                    ...
                    n._n = (bytesMultiplier ? bytesMultiplier : 1) * (stringOriginal.match(thousandRegExp) ? Math.pow(10, 3) : 1) * (stringOriginal.match(millionRegExp) ? Math.pow(10, 6) : 1) * (stringOriginal.match(billionRegExp) ? Math.pow(10, 9) : 1) * (stringOriginal.match(trillionRegExp) ? Math.pow(10, 12) : 1) * (string.indexOf("%") > -1 ? .01 : 1) * Number((string.indexOf("(") > -1 ? "-" : "") + string.replace(/[^0-9\.-]+/g, ""));
