define("lib/switchable/1.0.0/switchable-debug", [ "$-debug", "lib/widget/1.1.1/widget-debug", "lib/base/1.1.1/base-debug", "lib/class/1.1.0/class-debug", "lib/events/1.1.0/events-debug", "./plugins/effects-debug", "lib/easing/1.0.0/easing-debug", "./plugins/autoplay-debug", "./plugins/circular-debug", "./const-debug" ], function(require, exports, module) {
    // Switchable
    // -----------
    ...
    var Switchable = Widget.extend({
        ...
        _getPanelInfo: function(toIndex, fromIndex) {
            ...
            if (this.get('asyn') && $(toPanels).is(':empty')) {
                var url = triggers.eq(toIndex).data('url');
                $.ajax({
                    url: url,
                    cache: false,
                    beforeSend: function(XHR){
                        $(toPanels).html('<p class="ajax-loader" style="width:32px;height:32px;margin:12% auto 0;"><img src="http://static.n8n8.cn/Public/Images/common/ajax-loader.gif" alt="Loading……" /></p>')
                    },
                    success: function(r){
                        var toPanelsCnt = $(toPanels).html(r);
                    },
                    error: function(){
                        alert('数据加载错误，请您稍后再试！');
                    }
