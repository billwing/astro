define("uran/sweet/1.0.0/sweet-debug", [], function(require, exports, module) {
    /**
        ...
        tmpStr = tplStr.replace(re, function(m, text, expr) {
            ...
            if (expr != "") {
                if (expr.charAt(0) == "=") {
                    ...
                    if (!/[;\?\{\}:]/.test(expr.charAt(expr.length - 1))) expr = expr + ";";
                }
                replaced.push(expr);
            }
            return "";
   ...
   */
    function trim(text) {
        var rtrim = String.prototype.trim || function() {
            return (this + "").replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, "");
        };
        return text == null ? "" : rtrim.call(text);
    }
    /**
   * escape HTML
   * @param str
